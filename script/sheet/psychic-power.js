...
export class PsychicPowerSheet extends DarkHeresyItemSheet {
    ...
    async _prepareContext(options) {
        ...
        // under `damage`. Chips read the FULL weaponQualities map so a stored
        // quality outside the curated set still renders; only the add-dropdown
        // is curated to psychicPowerQualityKeys.
        const cfg = config.weaponQualities;
        const current = this.item.system.damage.specialQualities ?? [];
        ...
        // Add-dropdown options: curated psychic keys NOT already present, sorted
        // by localized label so the list reads alphabetically per locale.
        const present = new Set(current.map(q => q.key));
        const curated = new Set(config.psychicPowerQualityKeys);
    ...
    async _onFirstRender(context, options) {
        ...
        // Special-qualities chip editor. Three more delegated listeners on the
        // same persistent frame, mirroring the weapon sheet. Each resolves its
        // control via closest() and bails otherwise, so they ignore the rest of
        // the sheet — including the focus-autocomplete change events above. Array
        // mutations go through an explicit item.update({"system.damage.specialQualities":
        // next}) with a freshly rebuilt {key, value} array — NOT indexed named
        // inputs. The chips carry no `name`, so submitOnChange never includes
    ...
    /**
     * Apply a mutation to `system.damage.specialQualities`, serialized so
    ...
    #mutateQualities(mutator) {
        this.#qualityMutations = this.#qualityMutations.then(async () => {
            const current = (this.item.system.damage.specialQualities ?? []).map(q => ({ key: q.key, value: q.value }));
            const next = mutator(current);
            if (next) await this.item.update({ "system.damage.specialQualities": next });
